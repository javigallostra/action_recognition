            ...
            for event in active_events:
                for e in [(e[0],e[1]) for e in self.graph.edges() if self.graph.edge[e[0]][e[1]]['trigger'] == event]:
                    if e[1] in add_values:
                        add_values[e[1]].append(self.graph.node[e[0]]['value'] * self.graph.edge[e[0]][e[1]]['weight'] * self.graph.edge[e[0]][e[1]]['factor'])
                    else:
                        add_values[e[1]] = [self.graph.node[e[0]]['value'] * self.graph.edge[e[0]][e[1]]['weight'] * self.graph.edge[e[0]][e[1]]['factor']]
                ...
                for node in range(self.node_count):
                    ...
                    if node in add_values.keys():
                        self.graph.node[node]['value'] += sum(add_values[node])
                        #################
                        # SUM OR MAX???
                        #################
            self.plot()
